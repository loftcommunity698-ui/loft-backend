...
router.post("/:id/apply", optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (employerShouldNotify) {
      sendEmail({ type: "new_applicant", recipient: application.job.employer.email, data: { jobTitle: job.title, candidateName: applicantName } }).catch((sendErr) => log.error("New applicant email error", sendErr))
    }
    ...
    }).catch((sendErr) => log.error("Support inbox application notification error", sendErr))
